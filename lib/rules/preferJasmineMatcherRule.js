...
const isComparison = arg => (
  ts.isBinaryExpression(arg) &&
  OPERATORS.indexOf(arg.operatorToken.getText()) > -1
);

const isInsideBlock = node => findParentFromNode(node, parent => (
  ts.isCallExpression(parent) &&
  BLOCK_PATTERN.test(parent.expression.getText())
));
...
const walk = function walk(ctx) {
  const checkNode = function checkNode(node) {
    if (
      ...
      node.expression.getText() === 'expect' &&
      isInsideBlock(node)
    ) {
