...
const walk = function walk(ctx) {
    ...
    while (i < length) {
      ...
      if (
        ...
        const fix = new TSLint.Replacement(
          ...
          currentNode.getFullWidth(),
          `\n${currentNode.getFullText()}`
        );

        return ctx.addFailureAtNode(currentNode, Rule.FAILURE_STRING, fix);
      }
  ...
  const checkNode = function checkNode(node) {
    if (ts.isCallExpression(node) && IT_PATTERN.test(node.expression.getText())) {
      lastExpectNode = null;
