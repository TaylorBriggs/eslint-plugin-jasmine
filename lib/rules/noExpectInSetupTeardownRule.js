    ...
    if (callback && callback.body) {
      ...
      while (i < length) {
        ...
        if (Array.isArray(blacklist)) { // passed in via rule options
          functionName = trimSemicolon(childNode.getText(sourceFile));
          hasFailure = blacklist.indexOf(functionName) > -1;
        } else {
          functionName = childNode.getFirstToken(sourceFile).text;
          hasFailure = blacklist.test(functionName);
          functionName = hasFailure ? `${functionName}()` : functionName;
        }

        if (hasFailure) {
          const message = Rule.formatFailureMessage(functionName, setupFunctionName);

          ctx.addFailureAtNode(childNode, message);
        }
